...
interface KpiCardProps { label: string; value: number | string; sub?: string; accent?: boolean; icon?: React.ReactNode }
function KpiCard({ label, value, sub, accent, icon }: KpiCardProps) {
  return (
    <div className="bg-white rounded-lg shadow-sm p-5 flex flex-col gap-1" style={{ borderTop: `4px solid ${accent ? BRAND : "#d1d5db"}` }}>
...
interface StatusPillProps { label: string; value: number; color: string; hint: string }
function StatusPill({ label, value, color, hint }: StatusPillProps) {
  return (
...
export default function PublishedPage() {
  ...
  const overallNet = report
    ? report.overall.activeTotal - report.overall.positions.reduce((s, p) => s + p.prevTotal, 0)
    : 0;
  const totalJoiners = report?.overall.positions.reduce((s, p) => s + p.add, 0) ?? 0;
  const totalLeavers = report?.overall.positions.reduce((s, p) => s + p.resigned, 0) ?? 0;
  ...
  return (
    <div className="min-h-screen bg-[#f4f6f9]">
        ...
        <div className="max-w-7xl mx-auto px-6 py-3.5 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2.5">
              ...
              <span className="text-sm px-2.5 py-0.5 rounded-full font-bold bg-green-900/40 text-green-300">
                Published
              </span>
            </div>
          </div>
      ...
      {/* Published-at banner */}
      {publishedAt && (
        <div className="bg-green-50 border-b border-green-200 px-6 py-2.5 flex items-center gap-2 text-sm text-green-700 print:hidden">
          <Clock size={14} />
          Published on {new Date(publishedAt).toLocaleString("en-MY", { dateStyle: "long", timeStyle: "short" })}
        </div>
      )}
        ...
        {status === "not-found" && (
          <div className="bg-gray-50 border border-gray-200 rounded-lg p-10 text-center">
            ...
            <button
              ...
              className="text-sm font-bold px-5 py-2.5 rounded text-white"
              style={{ background: BRAND }}
            >
              Generate Report
            </button>
          ...
          <>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 mb-6">
              <KpiCard
                ...
                icon={<Users size={18} />}
              />
              {report.sites.map((sr: SiteReport) => (
                <KpiCard
                  key={sr.site}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
              <StatusPill label="B Status — Current Month" value={report.overall.bStatusCurrent} color="#c07000" hint="Serving notice, leaving this month" />
              <StatusPill label="B Status — Following Months" value={report.overall.bStatusFollowing} color="#b58000" hint="Serving notice, leaving next month+" />
              <StatusPill label="C Status — Left Already" value={report.overall.cStatus} color="#666" hint="Left, still in this month's payroll" />
            </div>

            <div className="bg-white rounded-lg shadow-sm mb-6 overflow-hidden" style={{ borderTop: `4px solid ${BRAND}` }}>
              <div className="px-6 py-4 border-b border-gray-200">
                <h3 className="brand-bar text-base font-bold text-gray-900 uppercase tracking-wide">Headcount by Position</h3>
              </div>
              <Tabs defaultValue="overall">
                ...
                <TabsContent value="overall" className="mt-0">
                  <HeadcountTable report={report.overall} prevWeekLabel={report.prevWeekLabel} currentWeekLabel={report.weekLabel} prevDate={report.prevDate} currentDate={report.reportDate} />
                </TabsContent>
                {report.sites.map((sr: SiteReport) => (
                  <TabsContent key={sr.site} value={sr.site} className="mt-0">
                    <HeadcountTable report={sr} prevWeekLabel={report.prevWeekLabel} currentWeekLabel={report.weekLabel} prevDate={report.prevDate} currentDate={report.reportDate} />
                  </TabsContent>
                ))}
              </Tabs>
            </div>
            ...
            <div className="bg-white rounded-lg shadow-sm overflow-hidden" style={{ borderTop: `4px solid ${BRAND}` }}>
              <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
                <h3 className="brand-bar text-base font-bold text-gray-900 uppercase tracking-wide">Resignation &amp; Movement</h3>
                <div className="flex gap-5 text-sm font-semibold text-gray-700">
                  <span className="flex items-center gap-1.5"><TrendingUp size={15} className="text-green-600" /> Joiners: <strong className="text-green-700">{totalJoiners}</strong></span>
                  <span className="flex items-center gap-1.5"><TrendingDown size={15} style={{ color: BRAND }} /> Leavers: <strong style={{ color: BRAND }}>{totalLeavers}</strong></span>
                </div>
              </div>
